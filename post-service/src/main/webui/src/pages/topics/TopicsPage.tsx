  ...
  return (
    <main className="topics-page">
      <header className="topics-page__intro">
        <div><p>Discovery &amp; filing</p><h1>Topics desk</h1></div>
        <p>Keep story categories useful, distinct and easy for readers to scan.</p>
      </header>

      <dl className="topic-totals" aria-label="Topic totals">
      ...
      {error ? <div className="topic-error" role="alert"><span>{error.message}</span><button type="button" onClick={() => void load()}>Reload topics</button></div> : null}
      {topics === null ? <div className="topic-loading" aria-live="polite">Reading the topic catalogue…</div> : <TopicLedger topics={topics} savingIds={savingIds} onSetActive={setActive} />}
    </main>
  );
}
