...
1. Read the relevant code and `AGENTS.md` so the plan fits the existing architecture (DDD package
...
- Implement **only against the contract** — the signatures, data shapes and behaviour exactly as
  written, placed in the correct domain package / feature folder per `AGENTS.md` (public face at
...
- Keep `app/` thin and the backend domain boundaries intact (`AGENTS.md`) — review the subagents'
