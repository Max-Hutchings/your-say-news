...
describe("castVote", () => {
  it("posts the postId and stance to /votes and returns the created vote", async () => {
    mockPost.mockResolvedValue({ data: { id: 9, postId: 3, voteFor: true } });

    const vote = await castVote(3, true);

    expect(mockPost).toHaveBeenCalledWith("http://posts.local:8082/votes", {
      postId: 3,
      voteFor: true,
    });
    expect(vote).toEqual({ id: 9, postId: 3, voteFor: true });
  });

  it("never sends a userId — identity is taken from the token server-side", async () => {
    mockPost.mockResolvedValue({ data: { id: 1, postId: 3, voteFor: false } });

    await castVote(3, false);

    const body = mockPost.mock.calls[0][1] as Record<string, unknown>;
    expect(Object.keys(body).sort()).toEqual(["postId", "voteFor"]);
  });
});

describe("getMyVote", () => {
  it("returns the caller's vote when the service responds 200", async () => {
    mockGet.mockResolvedValue({ status: 200, data: { id: 9, postId: 3, voteFor: false } });

    const vote = await getMyVote(3);

    expect(mockGet).toHaveBeenCalledWith("http://posts.local:8082/votes/3/mine");
    expect(vote).toEqual({ id: 9, postId: 3, voteFor: false });
  });
